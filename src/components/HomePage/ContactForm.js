...
const ContactForm = ({ course, formData, onClose }) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            let errorMessage = "An error occurred while submitting. Please try again."; // Default
            ...
            if (axios.isAxiosError(error)) {
                ...
                if (error.response) {
                    ...
                    if (status === 400) {
                        ...
                        if (responseData && typeof responseData === 'object' && responseData.message) {
                            ...
                            // 400 status, but unexpected data format
                            console.warn("Received 400 status, but 'message' field missing or data format incorrect in response.");
                            errorMessage = "Submission failed. Please check your input values.";
                        }
                    } else {
                        // Handle other error statuses (500, 404, 403 etc.)
                        errorMessage = `Submission failed due to a server issue (Status: ${status}). Please try again later.`;
                    }
                } else if (error.request) {
                    // Request was made, but no response received (network error, CORS preflight fail?)
                ...
                // Error is not from Axios (e.g., error within .then, state update etc.)
    ...
    const selectedCountryInfo = countryCodes.find((c) => c.code === formValues.countryCode);
    ...
    const buttonText = rawButtonText.includes("Demo Demo") ? rawButtonText.replace(/Demo\s*Demo/, "Demo").trim() : rawButtonText;

    return (
        <div className={styles.modalOverlay}>
            <div className={`${styles.modalContent} ${isInView ? styles.backgroundLoaded : ""}`} ref={formRef}>
                <span className={styles.closeBtnContact} onClick={onClose} role="button" aria-label="Close form">&times;</span>
                <form className={styles.contactForm} onSubmit={handleSubmit} noValidate>
                    ...
                    <div className={styles.contactFormGroup}>
                        ...
                        <div className={styles.inputWithIcon}>
                            <input type="text" id="name" name="name" value={formValues.name} onChange={handleChange} placeholder="Your Name" className={errors.name ? styles.inputError : ""} aria-invalid={!!errors.name} aria-describedby={errors.name ? "name-error" : undefined} required />
                    ...
                    {/* Email */}
                    <div className={styles.contactFormGroup}>
                         <label htmlFor="email" className={styles.srOnly}>Your Email</label>
                        <div className={styles.inputWithIcon}>
                            <input type="email" id="email" name="email" value={formValues.email} onChange={handleChange} placeholder="Your Email" className={errors.email ? styles.inputError : ""} aria-invalid={!!errors.email} aria-describedby={errors.email ? "email-error" : undefined} required />
                    ...
                    {/* Contact */}
                     <div className={styles.contactFormGroup}>
                        <label htmlFor="countryCode" className={styles.srOnly}>Country Code</label>
                        <label htmlFor="contact" className={styles.srOnly}>Contact Number</label>
                        <div className={styles.inputWithIcon}>
                            <select id="countryCode" name="countryCode" value={formValues.countryCode} onChange={handleChange} className={styles.selectCountryCode} aria-label="Select your country code">
                                {countryCodes.map(({ code, country }) => ( <option key={code} value={code}> {`${code} (${country})`} </option> ))}
                            </select>
                            <input type="tel" inputMode="numeric" id="contact" name="contact" value={formValues.contact} onChange={handleChange} placeholder={contactPlaceholder} className={`${styles.contactInput} ${errors.contact ? styles.inputError : ""}`} maxLength={selectedCountryInfo?.maxLength || 15} aria-invalid={!!errors.contact} aria-describedby={errors.contact ? "contact-error" : undefined} required />
                            <IonIcon icon={callOutline} aria-hidden="true"/>
                        </div>
                        {errors.contact && <span id="contact-error" className={styles.errorText} role="alert">{errors.contact}</span>}
                    </div>
                    {/* Submit Button */}
                ...
                {/* Thank You Popup */}
                {isThankYouVisible && ( <div className={styles.thankYouPopup} role="status"> <p>🎉 Thank you for submitting! We'll get back to you soon.</p> </div> )}
            </div>
        </div>
    );
};
