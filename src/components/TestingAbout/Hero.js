...
export default function Hero() {
  ...
  const vantaConfig = useMemo(() => {
    ...
    return {
      ...
      touchControls: true,
      gyroControls: false,
      minHeight: isMobile ? Math.min(window.innerHeight, 500) : isTablet ? 400 : 200,
      minWidth: isMobile ? Math.min(window.innerWidth, 400) : isTablet ? 600 : 200,
  ...
  const loadScript = useCallback((src, name) => {
  ...
  useEffect(() => {
    ...
    const initializeVanta = async () => {
      ...
      try {
        ...
        await Promise.race([
          loadScript('https://cdnjs.cloudflare.com/ajax/libs/vanta/0.5.24/vanta.waves.min.js', 'vanta'),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), 10000))
        ]);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 xl:gap-16 items-center">
            ...
            <div className="space-y-4 sm:space-y-5 md:space-y-6 lg:space-y-8">
              ...
              <div className="inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-yellow-400/10 border border-yellow-400/20 backdrop-blur-sm">
                <span className="text-yellow-400 text-xs sm:text-sm font-medium">🏆 #1 SAP Training Institute</span>
              </div>
              ...
              <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-300 leading-relaxed max-w-2xl">
                ...
                <span className="text-yellow-400 font-semibold"> Expert-led courses</span> with 
                <span className="text-yellow-400 font-semibold"> hands-on experience</span> to excel in the ERP industry.
              </p>
              ...
              <div className="grid grid-cols-3 gap-4 sm:gap-6 lg:gap-8 py-4 sm:py-6">
                <div className="text-center group">
                  <div className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-yellow-400 mb-1 group-hover:scale-110 transition-transform duration-300">No.1</div>
                  <div className="text-xs sm:text-sm md:text-base text-gray-400 leading-tight">Training Center</div>
                </div>
                
                <div className="text-center group">
                  <div className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-yellow-400 mb-1 group-hover:scale-110 transition-transform duration-300">5K+</div>
                  <div className="text-xs sm:text-sm md:text-base text-gray-400 leading-tight">Students Trained</div>
                </div>
                
                <div className="text-center group">
                  <div className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-yellow-400 mb-1 group-hover:scale-110 transition-transform duration-300">10+</div>
                  <div className="text-xs sm:text-sm md:text-base text-gray-400 leading-tight">Years Experience</div>
                </div>
              </div>
