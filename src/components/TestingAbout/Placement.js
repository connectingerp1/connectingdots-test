...
const SAPCompassDial = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveModule((prev) => {
        const currentIndex = prev ? modules.findIndex(m => m.id === prev.id) : -1;
  ...
  const CompassPoint = ({ module, isActive, isMobile }) => {
    ...
    const pointX = centerX + radius * Math.cos((module.angle - 90) * Math.PI / 180);
    const pointY = centerY + radius * Math.sin((module.angle - 90) * Math.PI / 180);
    ...
    return (
      <g>
        ...
        <text
          ...
          textAnchor="middle"
          fontSize={isActive ? (isMobile ? '16' : '25') : (isMobile ? '10' : '17')}
    ...
    return (
      <g>
        {fixedPositions.map((position, index) => {
          const pointX = centerX + radius * Math.cos((position.angle - 90) * Math.PI / 180);
          const pointY = centerY + radius * Math.sin((position.angle - 90) * Math.PI / 180);
          ...
          const moduleIndex = (index - rotationOffset + modules.length * 10) % modules.length;
          ...
          return (
            <g key={`label-${position.angle}`}>
              ...
              <text
                x={pointX}
                y={pointY + (isActive ? (isMobile ? 38 : 55) : (isMobile ? 28 : 42))}
                textAnchor="middle"
                fill="#E5E7EB"
                fontSize={isActive ? (isMobile ? '10' : '16') : (isMobile ? '8' : '12')}
              ...
              <text
                x={pointX}
                y={pointY + (isActive ? (isMobile ? 52 : 70) : (isMobile ? 40 : 56))}
                textAnchor="middle"
                fill={currentModule.color}
                fontSize={isActive ? (isMobile ? '9' : '14') : (isMobile ? '6' : '10')}
                fontWeight="bold"
                className="pointer-events-none select-none transition-all duration-500"
              >
    ...
    <div className="min-h-screen bg-black text-white p-4 md:p-8">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-lg md:text-xl text-gray-300 max-w-3xl mx-auto mb-6">
          Navigate your SAP journey with our interactive compass. Click any module to explore opportunities.
        </p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center gap-4 lg:gap-8 max-w-7xl mx-auto">
        ...
        <div className="relative">
          ...
          <svg width="550" height="550" viewBox="0 0 550 550" className="hidden md:block drop-shadow-2xl">
            {/* Outer Ring */}
            <circle
              cx="275"
              cy="275"
            ...
            <g 
              ...
              style={{ transform: `rotate(${rotation}deg)`, transformOrigin: '275px 275px' }}
            >
              <polygon
          ...
          <svg width="320" height="380" viewBox="0 0 320 380" className="block md:hidden drop-shadow-2xl">
            {/* Outer Ring */}
            <circle
              cx="160"
              cy="160"
            ...
            <div className="space-y-4 md:space-y-6">
              <div className="flex items-center gap-4">
                <div 
                  ...
                  style={{ backgroundColor: activeModule.color + '20', border: `2px solid ${activeModule.color}` }}
                >
                  {activeModule.icon}
                </div>
                <div>
                  <h3 className="text-xl md:text-2xl font-bold text-white">{activeModule.name}</h3>
                  <p className="text-sm md:text-base text-blue-100">{activeModule.description}</p>
                </div>
              </div>
              ...
              <div className="bg-[#1B263B] rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-gray-300 text-sm md:text-base">Success Rate</span>
                  <span className="text-2xl md:text-3xl font-bold" style={{ color: activeModule.color }}>
  ...
  >
    START LEARNING {activeModule.name}
            ...
            <div className="text-center py-8 md:py-12">
              ...
              <h3 className="text-xl md:text-2xl font-bold text-white mb-2">Choose Your Path</h3>
              <p className="text-sm md:text-base text-gray-400">
                Click on any module point on the compass to explore career opportunities and placement rates.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="mt-12 md:mt-16 grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 max-w-4xl mx-auto">
        <div className="text-center bg-gray-900 rounded-2xl p-4 md:p-6 border border-gray-700">
          <div className="text-3xl md:text-4xl font-bold text-blue-400 mb-2 font-mono">2500+</div>
          <div className="text-gray-300 uppercase tracking-wide text-xs md:text-sm">Students Guided</div>
        </div>
        <div className="text-center bg-gray-900 rounded-2xl p-4 md:p-6 border border-gray-700">
          <div className="text-3xl md:text-4xl font-bold text-green-400 mb-2 font-mono">95%</div>
          <div className="text-gray-300 uppercase tracking-wide text-xs md:text-sm">Average Success</div>
        </div>
        <div className="text-center bg-gray-900 rounded-2xl p-4 md:p-6 border border-gray-700">
          <div className="text-3xl md:text-4xl font-bold text-purple-400 mb-2 font-mono">100+</div>
          <div className="text-gray-300 uppercase tracking-wide text-xs md:text-sm">Partner Companies</div>
        </div>
      </div>
    </div>
  );
